...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%
	//a태그로 받아온 예약번호 값으로 특정한 예약 내역의 상세페이지 조회하기
	...
	//조건 걸 값 dto에 끼우기
	//1개의 레코드를 검색하는 read메서드에 dto 싣기
	ReservDTO dto = new ReservDTO();
// 	dto.setM_id(m_id);
	dto.setReserv_num(reserv_num2);
	//특정 예약번호의 레코드 1개를 읽어와 dto2에 싣기
	ReservDAO dao = new ReservDAO();
	ReservDTO dto2 = dao.read(dto);
	//dto2에서 필요한 값을 꺼내기
	//여기까지는 reservation DB에 값이 있었지만,
	String h_id = dto2.getH_id();
	String h_name = dto2.getH_name();
	//스탠다드, 프리미엄 객실 가격 값은 db에 없으므로
	//hotel_info DB에서 찾아와야한다.
	//HotelInfoDTO에 dto3를 만들고 dto2에서 꺼내온 h_id값을 싣는다.
	//(HotelInfoDAO의 info_read메서드는 h_id를 조건으로 레코드 1개를 검색)
	HotelInfoDTO dto3 = new HotelInfoDTO();
	dto3.setH_id(h_id);
	//HotelInfoDAO의 info_read메서드에 dto3을 입력값으로 전송
	//반환된 값(특정 h_id를 가진 레코드)을 dto4에 싣기 
	HotelInfoDAO dao2 = new HotelInfoDAO();
	HotelInfoDTO dto4 = dao2.info_read(dto3);
	//dto4에서 필요한 객실 가격 값을 꺼내오기
...
<!-- 좌측 메뉴바(회원정보, 예약정보) -->	
	...
	<!-- a태그로 받아온 예약번호 값을 form에 같이 넘겨주는 방법 -->
