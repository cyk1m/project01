...
<script type="text/javascript">
	...
	function dayDiff() {
		...
		date2 = $('#outId').val() //큰 수
		arr1 = date1.split('-') //arr1은 object(값이 저장된 주소 할당, Array는 object)
		arr2 = date2.split('-') //년,월,일을 분리
		d1 = new Date(arr1[0], arr1[1], arr1[2]) //d1은 object. Date 생성자 함수에 인수로 각각 넣어 Date객체로 만듦
		d2 = new Date(arr2[0], arr2[1], arr2[2])
		//object끼리 객체끼리 연산을 하면 자동형변환으로 객체는 원시값으로 변환된다.(객체는 숫자/문자형으로만 형변환됨)
		//그래서 line93에서 d2-d1을 했을 때 두 날짜의 시간차이가 숫자형으로 반환되는 것이다.
		diff = d2 - d1 //날짜 차이 알아내기. number
		currDay = 24 * 60 * 60 * 1000 //하루24시간을 밀리초로 변환. 이걸로 나누면 하루단위 구할 수 있다. 시*분*초*밀리세컨. number(기본형)
		DateDiff = parseInt(diff / currDay) //일수 차이. number
	}
...
<!-- 객실 내부 이미지(Slide) -->
<div class="section">
	<!-- radio에 동일한 name값을 주어 그룹핑한다(1개만 체크되는 효과) -->
	<!-- label에 연결하여 사용 가능 -->
